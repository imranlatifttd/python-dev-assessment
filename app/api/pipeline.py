            ...
            jsonify(
                {
                    ...
                    "details": [{"msg": str(e), "type": "execution_failed"}],
                }
            ),
            500,
        )
    ...
    if not run:
        return (
            jsonify(
                {
                    "error": "Not Found",
                    "details": [{"msg": "Run not found", "type": "resource_missing"}],
                }
            ),
            404,
        )
...
def get_profile_queries(profile_uuid):
    ...
    queries = (
        db_session.query(DiscoveredQuery)
    ...
    response_data = [
        DiscoveredQueryResponse.model_validate(q).model_dump(mode="json")
        for q in queries
    ]
...
def get_profile_recommendations(profile_uuid):
    ...
    recs = (
        db_session.query(ContentRecommendation)
        .filter_by(profile_uuid=profile_uuid)
        .all()
    )

    response_data = [
        ContentRecommendationResponse.model_validate(r).model_dump(mode="json")
        for r in recs
    ]
