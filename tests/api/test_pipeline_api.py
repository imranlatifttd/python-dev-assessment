...
def test_trigger_analysis_success(client, db, mocker):
    ...
    # mock the pipeline service so we don't actually run LLM/HTTP calls
    # as we just want to test the endpoint routing and response serialization
    mock_run_uuid = uuid.uuid4()

    # create a dummy PipelineRun in the DB so the endpoint can fetch it and serialize it
